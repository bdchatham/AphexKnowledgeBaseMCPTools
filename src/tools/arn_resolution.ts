...
import type { ResolveArnInput, ResolveArnOutput, ResolvedArn } from "../types/tools.js";
...
async function resolveCodeArn(
  ...
  workspaceBasePath: string
): Promise<{ filePath: string; lineNumber?: number; error?: string }> {
  ...
  const fullFilePath = resolvePath(
    workspaceBasePath,
    parsed.package,
    parsed.path
  );
  ...
  const lineNumber = await lookupSymbolLineNumber(
    parsed.package,
    parsed.path,
    parsed.symbol,
    workspaceBasePath
 ...
 */
export function convertToArchonDocPath(sourcePath: string): string {
 ...
 */
async function lookupSymbolLineNumber(
  packageName: string,
  filePath: string,
  symbolName: string,
  workspaceBasePath: string
 ...
 * @returns The line number if found, undefined otherwise
 */
async function searchScipIndexForSymbol(
  indexPath: string,
  filePath: string,
  symbolName: string,
  packageName: string
 ...
 */
export async function resolveArn(
  input: ResolveArnInput,
  options?: ResolveArnOptions
): Promise<ResolveArnOutput> {
  const { arn } = input;
