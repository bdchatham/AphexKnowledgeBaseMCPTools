...
export async function documentWorkspace(
  input: DocumentWorkspaceInput
): Promise<DocumentWorkspaceOutput> {
  ...
  const packagesToDocument = await orderPackagesByDependencies(
    resolvedWorkspacePath,
    filteredPackages
  );

  // Track results
  const packagesDocumented: PackageDocResult[] = [];
