...
import {
  ...
  extractIndexHash,
} from "../lib/doc_generator.js";
import type {
  GenerateArchonDocInput,
 ...
 */
async function readScipMetadata(
  packagePath: string
): Promise<{ indexes: Array<{ language: string; indexFile: string; hash: string }> } | null> {
  const metadataPath = join(packagePath, ".archon", "scip", "metadata.json");

  try {
    const content = await readFile(metadataPath, "utf-8");
    return JSON.parse(content) as {
      indexes: Array<{ language: string; indexFile: string; hash: string }>;
    };
  } catch {
    return null;
  }
 ...
 */
function groupSymbolsByFile(symbols: ScipSymbol[]): Map<string, ScipSymbol[]> {
 ...
 */
function filterRelationshipsForFile(
  relationships: ScipRelationship[],
  fileSymbols: ScipSymbol[]
): ScipRelationship[] {
  const symbolArns = new Set(fileSymbols.map((s) => s.arn));

  return relationships.filter(
    (rel) => symbolArns.has(rel.from) || symbolArns.has(rel.to)
  );
}
 ...
 */
function addIndexHashMarker(content: string, hash: string): string {
 ...
 */
export async function generateArchonDoc(
  input: GenerateArchonDocInput
): Promise<GenerateArchonDocOutput> {
  const { packagePath, files, force = false } = input;
  const resolvedPath = resolve(packagePath);

  const errors: string[] = [];
  const filesGenerated: GeneratedFileInfo[] = [];
  ...
  for (const indexPath of indexPaths) {
    ...
    for (const filePath of filesToProcess) {
      ...
      const fileRelationships = filterRelationshipsForFile(
        parseResult.relationships,
        fileSymbols
      );

      // Generate documentation
      const generatedDoc = generateForFile(
        filePath,
        fileSymbols,
        fileRelationships
      );
  ...
  const output: GenerateArchonDocOutput = {
    success,
    packagePath: resolvedPath,
    filesGenerated,
    filesSkipped,
