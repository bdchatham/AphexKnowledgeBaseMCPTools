 ...
 */
export async function discoverPackages(workspacePath: string): Promise<string[]> {
 ...
 */
async function parseGoModDependencies(goModPath: string): Promise<string[]> {
  try {
 ...
 */
async function parsePackageJsonDependencies(
...
export function topologicalSort(
   ...
   */
  function visit(packagePath: string): void {
 ...
 * @returns Sorted array of package paths (dependencies first)
 */
export async function orderPackagesByDependencies(
  workspacePath: string,
  packages: string[]
): Promise<string[]> {
...
function filterPackagesByName(
  ...
  return allPackages.filter((packagePath) => {
    ...
    return (
      filterSet.has(packageName) ||
      filterSet.has(normalizedPath) ||
      filterPackages.some(
        (filter) =>
          normalizedPath.endsWith(filter.toLowerCase()) ||
 ...
 */
export async function hasPackageChanged(packagePath: string): Promise<boolean> {
...
export async function indexWorkspace(
  ...
  // Dependencies are processed before dependents
  const packagesToIndex = await orderPackagesByDependencies(
    resolvedWorkspacePath,
    filteredPackages
  );

  // Track results
  const packagesIndexed: PackageIndexResult[] = [];
  ...
  for (const packagePath of packagesToIndex) {
    ...
    try {
      ...
      if (result.success && result.indexes.length > 0) {
        ...
        const packageSymbolCount = result.indexes.reduce(
          (sum, idx) => sum + idx.symbolCount,
          0
        );
