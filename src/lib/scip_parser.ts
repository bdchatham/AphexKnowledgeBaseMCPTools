...
): ScipParseErrorExtended {
  const error: ScipParseErrorExtended = {
    message,
    recoverable,
    code,
  };
...
const SCIP_KIND_MAP: Record<number, SymbolKind> = {
  7: "class", // Class
 ...
 */
function validateIndexStructure(index: unknown): ScipParseErrorExtended[] {
  const errors: ScipParseErrorExtended[] = [];

  if (!index || typeof index !== "object") {
    errors.push(
      createParseError(
        ScipErrorCode.INVALID_INDEX_STRUCTURE,
        "SCIP index is null or not an object",
        false,
  ...
  if (indexObj["documents"] !== undefined && !Array.isArray(indexObj["documents"])) {
    errors.push(
      createParseError(
        ScipErrorCode.INVALID_INDEX_STRUCTURE,
        "SCIP index 'documents' field is not an array",
        true,
        { context: { receivedType: typeof indexObj["documents"] } }
      )
    );
  }
  ...
  if (
    indexObj["externalSymbols"] !== undefined &&
    !Array.isArray(indexObj["externalSymbols"])
  ) {
    errors.push(
      createParseError(
        ScipErrorCode.INVALID_INDEX_STRUCTURE,
        "SCIP index 'externalSymbols' field is not an array",
        true,
        { context: { receivedType: typeof indexObj["externalSymbols"] } }
      )
    );
  }
...
function validateDocument(
  ...
  if (!doc || typeof doc !== "object") {
    errors.push(
      createParseError(
        ScipErrorCode.INVALID_INDEX_STRUCTURE,
        `Document at index ${docIndex} is null or not an object`,
        true,
        { context: { docIndex, receivedType: typeof doc } }
      )
    );
    return errors;
  }
  ...
  if (
    docObj["relativePath"] !== undefined &&
    typeof docObj["relativePath"] !== "string"
  ) {
    errors.push(
      createParseError(
        ScipErrorCode.MISSING_REQUIRED_FIELD,
        `Document at index ${docIndex} has invalid 'relativePath' field`,
        true,
        {
          context: {
            docIndex,
  ...
  if (
    docObj["occurrences"] !== undefined &&
    !Array.isArray(docObj["occurrences"])
  ) {
    errors.push(
      createParseError(
        ScipErrorCode.INVALID_INDEX_STRUCTURE,
        `Document at index ${docIndex} has invalid 'occurrences' field`,
        true,
        {
  ...
  if (docObj["symbols"] !== undefined && !Array.isArray(docObj["symbols"])) {
    errors.push(
      createParseError(
        ScipErrorCode.INVALID_INDEX_STRUCTURE,
        `Document at index ${docIndex} has invalid 'symbols' field`,
        true,
        {
          file: typeof docObj["relativePath"] === "string" ? docObj["relativePath"] : undefined,
 ...
 */
function validateSymbolFormat(
  symbol: unknown,
  filePath: string
): ScipParseErrorExtended | null {
  if (symbol === undefined || symbol === null) {
    return createParseError(
      ScipErrorCode.INVALID_SYMBOL_FORMAT,
      "Symbol is null or undefined",
      true,
      { file: filePath, context: { symbol } }
    );
  }
  ...
  if (typeof symbol !== "string") {
    return createParseError(
      ScipErrorCode.INVALID_SYMBOL_FORMAT,
      `Symbol is not a string, received ${typeof symbol}`,
      true,
      { file: filePath, context: { receivedType: typeof symbol } }
    );
  }
...
function validateOccurrenceRange(
  ...
  if (!range) {
    return createParseError(
      ScipErrorCode.INVALID_OCCURRENCE_RANGE,
      "Occurrence range is null or undefined",
      true,
      { file: filePath, context: { symbol } }
    );
  }
  ...
  if (!Array.isArray(range)) {
    return createParseError(
      ScipErrorCode.INVALID_OCCURRENCE_RANGE,
      `Occurrence range is not an array, received ${typeof range}`,
      true,
      { file: filePath, context: { symbol, receivedType: typeof range } }
    );
  }
  ...
  if (range.length < 3) {
    return createParseError(
      ScipErrorCode.INVALID_OCCURRENCE_RANGE,
      `Occurrence range has insufficient elements (${range.length}, expected 3 or 4)`,
      true,
      { file: filePath, context: { symbol, rangeLength: range.length } }
    );
  }

  // Validate that range elements are numbers
  for (let i = 0; i < range.length; i++) {
    if (typeof range[i] !== "number" || isNaN(range[i] as number)) {
      return createParseError(
        ScipErrorCode.INVALID_OCCURRENCE_RANGE,
        `Occurrence range element at index ${i} is not a valid number`,
        true,
        {
          file: filePath,
 ...
 */
function parseScipSymbolName(scipSymbol: string): string {
  if (!scipSymbol) {
    return "";
  }
 ...
 */
function mapScipKind(kind: number | undefined): SymbolKind {
 ...
 */
function extractFileFromSymbol(scipSymbol: string): string | null {
 ...
 */
function parseOccurrenceRange(range: number[]): SymbolLocation {
...
function extractRelationships(
  ...
  symbolMap: Map<string, ScipSymbol>,
  workspace: string,
  packageName: string
): ScipRelationship[] {
...
function findEnclosingDefinition(
  ...
  let bestMatch: ScipOccurrence | null = null;
  let bestDistance = Infinity;

  for (const occ of occurrences) {
    if (!occ.range || occ.range.length < 3) continue;
    if (!isDefinition(occ.symbolRoles || 0)) continue;
    if (occ.symbol === targetOcc.symbol) continue;

    const occLine = occ.range[0];
    if (occLine === undefined) continue;
 ...
 */
function generateArnForScipSymbol(
  scipSymbol: string,
  workspace: string,
  packageName: string,
  filePath: string
...
// Type definitions for parsed SCIP protobuf messages
...
interface ScipRelationshipProto {
  symbol: string;
  isReference?: boolean;
...
interface ScipSymbolInfo {
  ...
  relationships?: ScipRelationshipProto[];
  kind?: number;
  displayName?: string;
 ...
 */
function computeDocumentHash(doc: ScipDocument): string {
...
function processDocument(
  ...
  for (const symbolInfo of doc.symbols || []) {
    ...
    try {
      ...
      if (defOcc) {
        ...
        const rangeError = validateOccurrenceRange(
          defOcc.range,
          filePath,
          symbolInfo.symbol
        );
      ...
      errors.push(
        createParseError(
          ScipErrorCode.DOCUMENT_PROCESSING_ERROR,
          `Error processing symbol '${symbolInfo.symbol}': ${errorMessage}`,
          true,
          { file: filePath, context: { symbol: symbolInfo.symbol } }
        )
      );
    }
  }
  ...
  for (const occ of doc.occurrences || []) {
    if (!isDefinition(occ.symbolRoles || 0)) continue;
    if (symbolMap.has(occ.symbol)) continue;
    ...
    try {
      ...
      errors.push(
        createParseError(
          ScipErrorCode.DOCUMENT_PROCESSING_ERROR,
          `Error processing occurrence '${occ.symbol}': ${errorMessage}`,
          true,
          { file: filePath, context: { symbol: occ.symbol } }
        )
      );
    }
  }
...
export async function parse(
  ...
  try {
    ...
    errors.push(
      createParseError(
        ScipErrorCode.FILE_READ_ERROR,
        `Failed to read SCIP index file: ${errorMessage}`,
        false,
        {
          file: indexPath,
  ...
  try {
    ...
    errors.push(
      createParseError(
        ScipErrorCode.PROTOBUF_DECODE_ERROR,
        `Failed to load protobuf schema: ${errorMessage}`,
        false,
        { context: { protoPath: PROTO_PATH } }
      )
    );
  ...
  const hasNonRecoverableStructureError = structureErrors.some(
    (e) => !e.recoverable
  );
  ...
  const symbolMap = new Map<string, ScipSymbol>();
  const symbols: ScipSymbol[] = [];
  ...
  for (let docIndex = 0; docIndex < documents.length; docIndex++) {
    ...
    try {
      ...
      errors.push(
        createParseError(
          ScipErrorCode.DOCUMENT_PROCESSING_ERROR,
          `Failed to compute hash for document: ${errorMessage}`,
          true,
          { file: filePath, context: { docIndex } }
        )
      );
    }

    // Process the document and extract symbols
    const docSymbols = processDocument(
      doc,
      workspace,
      packageName,
      symbolMap,
  ...
  for (const extSymbol of externalSymbols) {
    if (!extSymbol || symbolMap.has(extSymbol.symbol)) continue;
    ...
    try {
      ...
      errors.push(
        createParseError(
          ScipErrorCode.DOCUMENT_PROCESSING_ERROR,
          `Error processing external symbol '${extSymbol.symbol}': ${errorMessage}`,
          true,
          { context: { symbol: extSymbol.symbol } }
        )
      );
    }
  }
  ...
  let relationships: ScipRelationship[] = [];
  try {
    ...
    errors.push(
      createParseError(
        ScipErrorCode.RELATIONSHIP_EXTRACTION_ERROR,
        `Failed to extract relationships: ${errorMessage}`,
        true,
...
export async function parseIncremental(
  ...
  try {
    ...
    errors.push(
      createParseError(
        ScipErrorCode.FILE_READ_ERROR,
        `Failed to check if index changed: ${errorMessage}`,
        true,
        { file: indexPath }
      )
    );
  ...
  try {
    ...
    errors.push(
      createParseError(
        ScipErrorCode.FILE_READ_ERROR,
        `Failed to read SCIP index file for incremental parse: ${errorMessage}`,
        true,
        { file: indexPath }
      )
    );
  ...
  try {
    ...
    errors.push(
      createParseError(
        ScipErrorCode.PROTOBUF_DECODE_ERROR,
        `Failed to decode SCIP index for incremental parse: ${errorMessage}`,
        true,
        { file: indexPath }
      )
    );
  ...
  const deletedFiles = new Set<string>(Object.keys(previousResult.fileHashes));
  ...
  for (const doc of documents) {
    ...
    try {
      ...
      errors.push(
        createParseError(
          ScipErrorCode.DOCUMENT_PROCESSING_ERROR,
          `Failed to compute hash for document '${filePath}': ${errorMessage}`,
          true,
          { file: filePath }
        )
      );
  ...
  try {
    ...
    errors.push(
      createParseError(
        ScipErrorCode.RELATIONSHIP_EXTRACTION_ERROR,
        `Failed to extract relationships during incremental parse: ${errorMessage}`,
        true,
        { context: { symbolCount: symbols.length } }
      )
    );
  ...
  const unchangedArns = new Set(
    previousResult.symbols
      .filter(
        (s: ScipSymbol) =>
          !changedFiles.has(s.location.file) &&
          !deletedFiles.has(s.location.file)
      )
      .map((s: ScipSymbol) => s.arn)
  );
