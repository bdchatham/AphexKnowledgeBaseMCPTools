...
export class VectorClient {
  ...
  private generateChunkId(arn: string, chunkIndex: number): string {
      ...
      if (end < content.length) {
        ...
        if (sentenceEnd > halfTarget) {
          ...
          if (newlineEnd > halfTarget) {
            ...
            if (spaceEnd > halfTarget) end = spaceEnd + 1;
          }
        }
      }
  ...
  async upsertChunks(chunks: DocChunk[]): Promise<number> {
    ...
    for (let i = 0; i < chunks.length; i += UPSERT_BATCH_SIZE) {
      ...
      const response = await fetch(
        `${this.qdrantUrl}/collections/${COLLECTION_NAME}/points`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  async pruneStaleChunks(
    packageName: string,
    currentArns: string[]
  ): Promise<number> {
    const countResponse = await fetch(
      `${this.qdrantUrl}/collections/${COLLECTION_NAME}/points/count`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
