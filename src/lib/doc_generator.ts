 ...
 */
const FUNCTION_TEMPLATE: DocTemplate = {
  kind: "function",
  template: `# {{name}}
 ...
 */
const CLASS_TEMPLATE: DocTemplate = {
  kind: "class",
  template: `# {{name}}
 ...
 */
const METHOD_TEMPLATE: DocTemplate = {
  kind: "method",
  template: `# {{name}}
 ...
 */
const MODULE_TEMPLATE: DocTemplate = {
  kind: "module",
  template: `# {{name}}
 ...
 */
const VARIABLE_TEMPLATE: DocTemplate = {
  kind: "variable",
  template: `# {{name}}
 ...
 */
const TYPE_TEMPLATE: DocTemplate = {
  kind: "type",
  template: `# {{name}}
 ...
 */
const TEMPLATES: Record<SymbolKind, DocTemplate> = {
 ...
 */
export function getTemplateForKind(kind: SymbolKind): DocTemplate {
...
function buildRelationshipsSection(
  ...
  symbolArn: string
): { content: string; referencedArns: string[] } {
  const grouped = groupRelationshipsByType(relationships, symbolArn);
  const referencedArns: string[] = [];
 ...
 */
export function generateForSymbolWithTemplate(
  symbol: ScipSymbol,
  relationships: ScipRelationship[]
): GeneratedDoc {
  ...
  const { content: relationshipsContent, referencedArns } = buildRelationshipsSection(
    relationships,
    symbol.arn
  );
 ...
 */
function getKindDescription(kind: SymbolKind): string {
  const descriptions: Record<SymbolKind, string> = {
    function: "function",
    class: "class",
    method: "method",
    variable: "variable",
 ...
 */
function extractSymbolNameFromArn(arn: string): string {
 ...
 */
function generateDocPath(sourcePath: string): string {
 ...
 */
function generateArnLink(symbolName: string, arn: string): string {
...
function groupRelationshipsByType(
  relationships: ScipRelationship[],
  symbolArn: string
): Map<RelationshipType, string[]> {
  const grouped = new Map<RelationshipType, string[]>();
...
export function generateForSymbol(
  ...
  lines.push("");
  lines.push(
    `This ${getKindDescription(symbol.kind)} is defined in \`${sourcePath}\` at line ${symbol.location.line}.`
  );
  lines.push("");
  ...
  const groupedRelationships = groupRelationshipsByType(
    relationships,
    symbol.arn
  );
 ...
 */
function generateDocArn(filePath: string): string {
 ...
 */
function filterPublicSymbols(symbols: ScipSymbol[]): ScipSymbol[] {
 ...
 */
function groupSymbolsByKind(symbols: ScipSymbol[]): Map<SymbolKind, ScipSymbol[]> {
  const grouped = new Map<SymbolKind, ScipSymbol[]>();
 ...
 */
function getKindPluralDescription(kind: SymbolKind): string {
  const descriptions: Record<SymbolKind, string> = {
    function: "Functions",
    class: "Classes",
    method: "Methods",
 ...
 */
function generateSymbolSummary(symbol: ScipSymbol): string {
  if (symbol.documentation) {
 ...
 * @returns Generated documentation with content and ARN references
 */
export function generateForFile(
  filePath: string,
  symbols: ScipSymbol[],
  relationships: ScipRelationship[]
): GeneratedDoc {
  ...
  if (publicSymbols.length > 0) {
    ...
    for (const [kind, kindSymbols] of groupedSymbols) {
      ...
      for (const symbol of kindSymbols) {
        ...
        const symbolRelationships = relationships.filter(
          (rel) => rel.from === symbol.arn
        );
  ...
  const allRelationships = new Map<RelationshipType, Set<string>>();
  for (const symbol of symbols) {
    const symbolRels = relationships.filter((rel) => rel.from === symbol.arn);
    for (const rel of symbolRels) {
      const existing = allRelationships.get(rel.type) || new Set<string>();
      existing.add(rel.to);
 ...
 */
export function extractManualSections(doc: string): Array<{
  startMarker: string;
  endMarker: string;
  content: string;
  fullMatch: string;
 ...
 */
export function preserveManualSections(
  existingDoc: string,
  newDoc: string
): string {
  ...
  if (newSections.length > 0) {
    ...
    for (let i = 0; i < sectionsToReplace; i++) {
      ...
      if (existingSection && newSection) {
        ...
        const replacement =
          MANUAL_SECTION_START + existingSection.content + MANUAL_SECTION_END;
 ...
 */
export function extractIndexHash(content: string): string | null {
 ...
 */
export function isOrphaned(content: string): boolean {
 ...
 */
export async function markOrphaned(docPath: string): Promise<void> {
