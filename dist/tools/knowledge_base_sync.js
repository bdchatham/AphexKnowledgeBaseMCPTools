...
function filterPackagesByName(allPackages, filterPackages) {
    ...
    return allPackages.filter((packagePath) => {
        ...
        return (filterSet.has(name) ||
            ...
            filterPackages.some((f) => normalized.endsWith(f.toLowerCase()) || packagePath.includes(f)));
    });
}
...
export async function syncToKnowledgeBase(input) {
    ...
    for (const packagePath of filteredPackages) {
        ...
        try {
            ...
            for (const indexPath of indexPaths) {
                const parseResult = await parseScipIndex(indexPath, DEFAULT_WORKSPACE, packageName);
            ...
            // Sync to code graph
            const graphResult = await graphClient.syncFromScip(packagePath, DEFAULT_WORKSPACE, packageName, allSymbols, allRelationships, latestHash);
            ...
            for (const [filePath, fileSymbols] of symbolsByFile) {
                const fileRelationships = allRelationships.filter((r) => fileSymbols.some((s) => s.arn === r.from) ||
                    fileSymbols.some((s) => s.arn === r.to));
                const doc = generateForFile(filePath, fileSymbols, fileRelationships);
                currentArns.push(doc.arn);
                const chunks = vectorClient.chunkDocument(doc.content, doc.docPath, doc.arn, doc.referencedArns, packageName);
                if (chunks.length > 0) {
                    chunksUpserted += await vectorClient.upsertChunks(chunks);
                }
            }
            chunksPruned = await vectorClient.pruneStaleChunks(packageName, currentArns);
            packagesSynced.push({
                package: packageName,
...
//# sourceMappingURL=knowledge_base_sync.js.map