...
function registerTools(server) {
    ...
    server.tool("generate_scip_index", "Generate SCIP indexes for a package by detecting languages and invoking appropriate SCIP tools (scip-go, scip-typescript).", {
    ...
    server.tool("generate_archon_doc", "Generate plain English documentation from SCIP indexes with ARN references. Creates .archon.md files co-located with source files.", {
    ...
    // Tool 3: resolve_arn
    server.tool("resolve_arn", "Resolve an ARN to its file location and symbol information. Supports both code and doc ARN types.", {
    ...
    server.tool("index_workspace", "Orchestrate SCIP indexing across all packages in a workspace. Discovers packages, analyzes dependencies, and indexes in dependency order.", {
    ...
    server.tool("document_workspace", "Orchestrate documentation generation across all packages in a workspace. Only generates documentation for packages with changed SCIP indexes.", {
        workspacePath: z.string().describe("Path to the workspace root"),
        packages: z
            .array(z.string())
            .optional()
    ...
    server.tool("sync_to_knowledge_base", "Sync SCIP indexes and Archon documentation to the Knowledge Base. Orchestrates synchronization of code intelligence data to the Code Graph and Vector Store.", {
        workspacePath: z.string().describe("Path to the workspace root"),
        packages: z
            .array(z.string())
            .optional()
